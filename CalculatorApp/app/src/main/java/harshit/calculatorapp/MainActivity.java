package harshit.calculatorapp;

import android.content.Intent;
import android.os.Bundle;
import android.support.design.widget.FloatingActionButton;
import android.support.design.widget.Snackbar;
import android.support.v7.app.AppCompatActivity;
import android.support.v7.widget.Toolbar;
import android.view.View;
import android.view.Menu;
import android.view.MenuItem;
import android.widget.EditText;
import android.widget.Toast;

import com.fathzer.soft.javaluator.DoubleEvaluator;

import butterknife.BindView;
import butterknife.ButterKnife;
import butterknife.OnClick;

public class MainActivity extends AppCompatActivity {

    @BindView(R.id.editText)
    EditText editText;
    private int count = 0;
    private String expression;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        ButterKnife.bind(this);
        /*e1=(EditText)findViewById(R.id.editText1);
        e2=(EditText)findViewById(R.id.editText2);
        dbHelper=new DBHelper(this);
        e2.setText("0");*/

    }

    @OnClick(R.id.num0)
    public void click0() {
        clickedNumber("0");
    }


    @OnClick(R.id.num1)
    public void click1() {
        clickedNumber("1");
    }

    @OnClick(R.id.num2)
    public void click2() {
        clickedNumber("2");
    }

    @OnClick(R.id.num3)
    public void click3() {
        clickedNumber("3");
    }

    @OnClick(R.id.num4)
    public void click4() {
        clickedNumber("4");
    }

    @OnClick(R.id.num5)
    public void click5() {
        clickedNumber("5");
    }

    @OnClick(R.id.num6)
    public void click6() {
        clickedNumber("6");
    }

    @OnClick(R.id.num7)
    public void click7() {
        clickedNumber("7");
    }

    @OnClick(R.id.num8)
    public void click8() {
        clickedNumber("8");
    }

    @OnClick(R.id.num9)
    public void click9() {
        clickedNumber("9");
    }

    @OnClick(R.id.decimal)
    public void clickDecimal() {
        String lastChar = editText.getText().toString().substring(editText.getText().toString().length() - 1);
        if (lastChar.equals("-") || lastChar.equals("+") || lastChar.equals("*") || lastChar.equals("/")
                || lastChar.equals("%") || lastChar.equals("^")) {
            Toast.makeText(this, "Decimal cant be put after operators", Toast.LENGTH_SHORT).show();
        } else if (count == 0 && editText.length() != 0) {
            editText.setText(editText.getText() + ".");
            count++;
        }
    }

    @OnClick(R.id.ce)
    public void clickCE() {
        if (editText.getText().toString().length() > 1) {
            editText.setText(editText.getText().toString().substring(0, editText.getText().toString().length() - 1));
        } else {
            editText.setText("0");
        }
    }

    @OnClick(R.id.cl)
    public void clickCL() {
        editText.setText("0");
        count = 0;
        expression = "";
    }

    @OnClick(R.id.plus)
    public void clickPlus() {
        operationClicked("+");
    }

    @OnClick(R.id.minus)
    public void clickMinus() {
        operationClicked("-");
    }

    @OnClick(R.id.divide)
    public void clickDivide() {
        operationClicked("/");
    }

    @OnClick(R.id.multiply)
    public void clickMultiply() {
        operationClicked("*");
    }

    @OnClick(R.id.percent)
    public void clickPercent() {
        operationClicked("%");
    }

    @OnClick(R.id.power)
    public void clickPower() {
        operationClicked("^");
    }

    @OnClick(R.id.equal)
    public void clickEqual() {

        expression = editText.getText().toString();
        try {
            Double result = new ExtendedDoubleEvaluator().evaluate(expression);
            if (("" + result).equals("Infinity")) {
                Toast.makeText(this, "Result is infinity", Toast.LENGTH_SHORT).show();
                editText.setText("0");
                count = 0;
            } else {
                editText.setText("" + result);
                count = 1;
            }
        } catch (IllegalArgumentException e) {
            Toast.makeText(this, "Invalid Expression", Toast.LENGTH_SHORT).show();
        }

    }

    /*public void onClick(View v)
    {
        switch(v.getId())
        {
            case R.id.num0:
                e2.setText(e2.getText()+"0");
                break;

            case R.id.num1:
                e2.setText(e2.getText()+"1");
                break;

            case R.id.num2:
                e2.setText(e2.getText()+"2");
                break;

            case R.id.num3:
                e2.setText(e2.getText()+"3");
                break;


            case R.id.num4:
                e2.setText(e2.getText()+"4");
                break;

            case R.id.num5:
                e2.setText(e2.getText()+"5");
                break;

            case R.id.num6:
                e2.setText(e2.getText()+"6");
                break;

            case R.id.num7:
                e2.setText(e2.getText()+"7");
                break;

            case R.id.num8:
                e2.setText(e2.getText()+"8");
                break;

            case R.id.num9:
                e2.setText(e2.getText()+"9");
                break;

            case R.id.dot:
                if(count==0 && e2.length()!=0)
                {
                    e2.setText(e2.getText()+".");
                    count++;
                }
                break;

            case R.id.clear:
                e1.setText("");
                e2.setText("");
                count=0;
                expression="";
                break;

            case R.id.backSpace:
                text=e2.getText().toString();
                if(text.length()>0)
                {
                    if(text.endsWith("."))
                    {
                        count=0;
                    }
                    String newText=text.substring(0,text.length()-1);
                    //to delete the data contained in the brackets at once
                    if(text.endsWith(")"))
                    {
                        char []a=text.toCharArray();
                        int pos=a.length-2;
                        int counter=1;
                        //to find the opening bracket position
                        for(int i=a.length-2;i>=0;i--)
                        {
                            if(a[i]==')')
                            {
                                counter++;
                            }
                            else if(a[i]=='(')
                            {
                                counter--;
                            }
                            //if decimal is deleted b/w brackets then count should be zero
                            else if(a[i]=='.')
                            {
                                count=0;
                            }
                            //if opening bracket pair for the last bracket is found
                            if(counter==0)
                            {
                                pos=i;
                                break;
                            }
                        }
                        newText=text.substring(0,pos);
                    }
                    //if e2 edit text contains only - sign or sqrt at last then clear the edit text e2
                    if(newText.equals("-")||newText.endsWith("sqrt"))
                    {
                        newText="";
                    }
                    //if pow sign is left at the last
                    else if(newText.endsWith("^"))
                        newText=newText.substring(0,newText.length()-1);

                    e2.setText(newText);
                }
                break;

            case R.id.plus:
                operationClicked("+");
                break;

            case R.id.minus:
                operationClicked("-");
                break;

            case R.id.divide:
                operationClicked("/");
                break;

            case R.id.multiply:
                operationClicked("*");
                break;

            case R.id.sqrt:
                if(e2.length()!=0)
                {
                    text=e2.getText().toString();
                    e2.setText("sqrt(" + text + ")");
                }
                break;

            case R.id.square:
                if(e2.length()!=0)
                {
                    text=e2.getText().toString();
                    e2.setText("("+text+")^2");
                }
                break;

            case R.id.posneg:
                if(e2.length()!=0)
                {
                    String s=e2.getText().toString();
                    char arr[]=s.toCharArray();
                    if(arr[0]=='-')
                        e2.setText(s.substring(1,s.length()));
                    else
                        e2.setText("-"+s);
                }
                break;

            case R.id.equal:
                *//*for more knowledge on DoubleEvaluator and its tutorial go to the below link
                http://javaluator.sourceforge.net/en/home*//*//*
                if(e2.length()!=0)
                {
                    text=e2.getText().toString();
                    expression=e1.getText().toString()+text;
                }
                e1.setText("");
                if(expression.length()==0)
                    expression="0.0";
                DoubleEvaluator evaluator = new DoubleEvaluator();
                try
                {
                    //evaluate the expression
                    result=new ExtendedDoubleEvaluator().evaluate(expression);
                    //insert expression and result in sqlite database if expression is valid and not 0.0
                    if(!expression.equals("0.0"))
                        dbHelper.insert("STANDARD",expression+" = "+result);
                    e2.setText(result+"");
                }
                catch (Exception e)
                {
                    e2.setText("Invalid Expression");
                    e1.setText("");
                    expression="";
                    e.printStackTrace();
                }
                break;

            case R.id.openBracket:
                e1.setText(e1.getText()+"(");
                break;

            case R.id.closeBracket:
                e1.setText(e1.getText()+")");
                break;

            case R.id.history:
                Intent i=new Intent(this,History.class);
                i.putExtra("calcName","STANDARD");
                startActivity(i);
                break;
        }
    }*/

    private void clickedNumber(String s) {
        if (s.equals("0") && count == 1) {
            Toast.makeText(this, "Zeros after decimal is useless", Toast.LENGTH_SHORT).show();
            return;
        }

        if (!editText.getText().toString().equals("0"))
            editText.setText(editText.getText() + s);
        else editText.setText(s);

    }

    private void operationClicked(String op) {
        String lastChar = editText.getText().toString().substring(editText.getText().toString().length() - 1);

        if (op.equals("-") && editText.getText().toString().equals("0")) {
            editText.setText("-");
        } else if (lastChar.equals("-") || lastChar.equals("+") || lastChar.equals("*") || lastChar.equals("/")
                || lastChar.equals("%") || lastChar.equals("^")) {
            Toast.makeText(this, "Invalid Expression", Toast.LENGTH_SHORT).show();
        } else if (!editText.getText().toString().equals("0") && !editText.getText().toString().equals("-")) {
            editText.setText(editText.getText() + op);
            count = 0;
        } else {
            Toast.makeText(this, "Zero can be a operand", Toast.LENGTH_SHORT).show();
        }
    }
}
